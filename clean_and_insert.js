...
// First, clean up all stray reflections inside the outline (lines 1 to 150 approx)
let lines = txt.split('\n');
let outlineLines = lines.slice(0, 150);
let restLines = lines.slice(150);

let cleanedOutline = [];
let skip = false;
for (let i = 0; i < outlineLines.length; i++) {
    let line = outlineLines[i];
    if (line.includes('### A Moment to Build (Reflection)')) {
        // Look backwards and remove any stray "---" or "#" markers associated with it
        while (cleanedOutline.length > 0 && (cleanedOutline[cleanedOutline.length - 1].trim() === '---' || cleanedOutline[cleanedOutline.length - 1].trim() === '#' || cleanedOutline[cleanedOutline.length - 1].trim() === '')) {
            cleanedOutline.pop();
        }
        skip = true;
        continue;
    ...
    if (skip) {
        if (line.includes('**One step:**')) {
            skip = false;
        }
        continue;
    }
    cleanedOutline.push(line);
}

lines = cleanedOutline.concat(restLines);

// Now, insert the reflections before the next Part/Chapter headers in the rest of the text
let joined = lines.join('\n');

// Find the LAST occurrence of "# Part 3: The Anatomy of the Trap"
let parts = joined.split('# Part 3: The Anatomy of the Trap');
if (parts.length > 2) {
    let lastIndex = parts.length - 1;
    let before = parts.slice(0, lastIndex).join('# Part 3: The Anatomy of the Trap');
    let after = parts[lastIndex];
    joined = before + '\n\n' + ref7 + '\n\n# Part 3: The Anatomy of the Trap' + after;
}

// Find the LAST occurrence of "# Part 4: The Climb Out (How we get free)"
parts = joined.split('# Part 4: The Climb Out');
if (parts.length > 2) {
    let lastIndex = parts.length - 1;
    let before = parts.slice(0, lastIndex).join('# Part 4: The Climb Out');
    let after = parts[lastIndex];
    joined = before + '\n\n' + ref8 + '\n\n# Part 4: The Climb Out' + after;
}

// Find the LAST occurrence of "# Part 5: The Charge"
parts = joined.split('# Part 5: The Charge');
if (parts.length > 2) {
    let lastIndex = parts.length - 1;
    let before = parts.slice(0, lastIndex).join('# Part 5: The Charge');
    let after = parts[lastIndex];
    joined = before + '\n\n' + ref11 + '\n\n# Part 5: The Charge' + after;
}

fs.writeFileSync(filePath, joined, 'utf8');
console.log('Cleaned and inserted.');
